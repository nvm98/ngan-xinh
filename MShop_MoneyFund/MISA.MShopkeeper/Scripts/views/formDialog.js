...
class FormDialog {
    ...
    constructor() {
        ...
        //Người tạo NVMANH 17/6/2019
        this.fundID;
        //Kiểm tra phiếu thu / trả nợ
        //Người tạo NVMANH 17/7/2019
        this.CheckType;
        this.CheckTypeEdit;
        //Kiểm tra phiếu khác thu nợ
        //Người tạo NVMANH 17/7/2019
        this.CheckDifferent;
        this.CheckDifferentEdit;
    ...
    initEvent() {
        ...
        //Hàm autoComplete cho các combobox
        //NGười tạo NVMANH 30/7/2019
        this.autoCompleteAll();
        //sự kiện nhấn chuyên trang
        //Người tạo NVMANH 30/7/2019
        this.eventClickPaging();
    }
    //Hiện dialog khi nhấn các button trên thanh menu
    //Người tạo NVMANH 7/6/2019
    setButton() {
        ...
        $('.content-table-header input').on('focusout', this.evtClickFilter);
        //Bắt sự kiện phân trang
        //Người tạo NVMANH 30/7/2019
        $('.btn-paging-select select, #page-number').change(this.eventPaging);
    }
        ...
        if ($('.select-bill-collect .wrap-collect-table-body .col-table-body-check.checked').length > 0) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $.each(listVoucherID, function (index, item) {
                        listBillSelect.push(res.Data.find(x => x.BillID === item));
                    });
                    var totalBillDebt = 0;
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $('[fieldname="supplierReason"]').val("Thu nợ của " + data['SupplierName']);
                    $('.dialog-body-debt .input-detail-1').val("Thu nợ của " + data['SupplierName']);
                    $('.dialog-body-debt .input-detail-3').val("Thu nợ từ bán hàng");
                    //Chọn nhân viên đang đăng nhập mặc định là nhân viên thu
                    //Người tạo NVMANH 15/7/2019
                    $('.dialog-body-debt [fieldname="EmployeeCode"]').val("NV0005");
        ...
        if (rowDetail.length > 0) {
            if ($('#col-total-money span').text() === '0') {
                ...
                if (form.checkAdd) {
                    ...
                    if ($('#dialog-add-receipt').hasClass('add-collect-money')) {
                        fund['CheckType'] = true;
                    }
                    if ($('#dialog-add-receipt').hasClass('add-pay-money')) {
                        fund['CheckType'] = false;
                    }
                    if ($('#radio1').is(":checked")) {
                        fund['CheckDifferent'] = true;
                    }
                    if ($('#radio2').is(":checked")) {
                        fund['CheckDifferent'] = false;
                    }
                    $.ajax({
                        method: "Post",
                        url: "/fund/",
                        ...
                        success: function (fund) {
                            ...
                            datatable.loadDataTableMain('/fund');
                            datatable.getTotal();
                        }
                    });
                    $.ajax({
                        method: "GET",
                        url: '/fund/id/' + fund['FundNumberVoucher'],
                        async: false,
                        success: function (res) {
                            let fundDetail = [];
                            var rowDetail = $('.table-detail-body .row-tab-detail-radio');
                            $.each(rowDetail, function (index, item) {
                                ...
                                detail['FundID'] = res.Data['FundID'];
                                fundDetail.push(detail);
                            });
                            form.addRowDetail(fundDetail);
                        },
                        error: function () {
                    ...
                    fund['CheckType'] = form.CheckTypeEdit;
                    fund['CheckDifferent'] = form.CheckDifferentEdit;
                    fund['FundDate'] = datatable.convertStringJSToStringCsharp($('.dialog-date-input'));
                    fund['FundNumberVoucher'] = $('#dialog-voucher-number-input').val();
                    ...
                    listDetailID = form.getIDDetailbyIDFund(form.fundID);
                    console.log(listDetailID);
                    ...
                    var listFundDetailAdd = [];
                    var listFundDetailEdit = [];
                  
                    $.each(listFundDetail, function (index, item) {
                        debugger
                        if (!item['FundDetailID'])
                        {
                            listFundDetailAdd.push(item);
                        }
                        else
                        {
                            listFundDetailEdit.push(item);
                        }
                        
                       listDetailID.splice(listDetailID.indexOf(item['FundDetailID']), 1);
                       
                    });
                    debugger
                    form.editFundDetail(listFundDetailEdit);
                    form.deleteFundDetail(listDetailID);
                    form.addRowDetail(listFundDetailAdd);
                }
                var debtInfo = $('.dialog-body-debt .dialog-info-submitter-input,.dialog-body-debt .dialog-info-address-input,.dialog-body-debt .dialog-info-reason-input,.dialog-body-debt .employee-input-left')
                debtInfo.addClass('debt-background');
                debtInfo.find('input').attr('class', "input-blinking debt-background");
    ...
    //Lấy danh sách ID của chi tiết hóa đơn theo mã hóa đơn
    //Người tạo NVMANH 17/7/2019
    getIDDetailbyIDFund(idFund) {
        var listIDDetail = [];
        $.ajax({
            async: false,
            method: 'GET',
            url: "/fund/detail/" + idFund,
            success: function (res) {
                var data = res.Data;
                $.each(data, function (index, item) {
                    listIDDetail.push(item['FundDetailID']);
                });
            }
        });
        return listIDDetail;
    ...
    //Sự kiện nhấn tải lại 
    //Người tạo NVMANH 30/7/2019
    eventBtnRefresh() {
        $('#page-number').val('1');
        var pageSize = $('.btn-paging-select option:selected').val();
        datatable.filterFund(form.getConditionFilter(), 1, pageSize);
    }
    //sự kiện nhấn chuyên trang
    //Người tạo NVMANH 30/7/2019
    eventClickPaging() {
        $('.icon-step-forward').click(function () {
            var number = parseInt($('#page-number').val());
        ...
        $('.icon-forward').click(function () {
            $('#page-number').val($('.total-page').text());
            form.eventPaging();
        });
    }
    //Sự kiện phân trang
    ...
    eventPaging() { 
        ...
        var pageSize = $('.btn-paging-select option:selected').val();
        datatable.filterFund(form.getConditionFilter(), pageNumber, pageSize);
    }
    //Cài đặt phím tắt cho các chức năng
    //Người tạo NVMANH 12/6/2019
    setAllHotKey() {
        $(window).keydown(function (event) {
            ...
            if (event.keyCode === 38) {
                if (!$("#dialog-add-receipt").dialog("isOpen") && !$("#dialog-delete").dialog("isOpen") && !form.autoCompleteIsOpen) {
    ...
    createIDCollect() {
        $.ajax({
            ...
            $('.dialog-voucher-number-input input').val(res.Data);
        }).fail(function () {
    ...
    dataDialogDelete() {
        if ($('.row-content-active').length > 1) {
            ...
            $.ajax({
                ...
                $('.content-alert-dialog span').text(res.Data['FundNumberVoucher']);
            }).fail(function () {
    ...
    loadDataToDialog() {
        ...
        $.ajax({
            ...
            form.CheckTypeEdit = data['CheckType'];
            form.CheckDifferentEdit = data['CheckDifferent'];
            //console.log(form.CheckTypeEdit);
            //console.log(form.CheckDifferentEdit);
            $("#dialog-info-reason-input").val(data['FundReason']);
            //Lấy dữ liệu từ bảng employ cho phiếu thu
            //Người tạo NVMANH 17/6/2019
            $("#employee-input-left").val(data['EmployeeCode']);
            $("#employee-input-right").val(data['EmployeeName']);
            //Lấy dữ liệu từ bảng nhà cung cấp
            //Người tạo NVMANH 17/6/2019
            $("#object-input-left").val(data['SupplierCode']);
    ...
    inputCollectMoney() {
        ...
        $('body').on('keyup', '.select-bill-collect #collect-input-3', function () {
            ...
            var count = $('.col-table-body-check.checked').length;
            if ($('.col-table-body-check.checked').length != 0 && $('.col-table-body-check.checked').length != $('.col-table-body-check').length) {
                $.each(arrayRow, function (index, item) {
                    if (!$(item).find('.col-table-body-check').hasClass('checked')) {
                        arrayRow.splice(index, 1);
                        arrayRow.push(item);
                    }
                });
            }
    ...
    evtBtnAgreeSupplier() {
        ...
        $('#object-input-left').val($(rowSelect).find('[fieldname="SupplierCode"]').text());
    ...
    evtBtnAgreeEmployee() {
        ...
        if ($('#radio1').is(":checked")) {
            $('.dialog-body-different [fieldname="EmployeeCode"]').val($(rowSelect).find('[fieldname="EmployeeCode"]').text());
        ...
        if ($('#radio2').is(":checked")) {
            $('.dialog-body-debt [fieldname="EmployeeCode"]').val($(rowSelect).find('[fieldname="EmployeeCode"]').text());
    ...
    autocompleteSupplier() {
        ...
        $.ajax({
            ...
            success: function (data) {
                array = data.Data.map(function (item) {
                    return {
        ...
        $("#object-input-left").autocomplete({
            ...
            select: function (event, ui) {
                $("#object-input-left").val(ui.item.SupplierCode);
            ...
            return $("<li class='li-supplier'>")
                ...
                .append("<div class='supplierId'>" + item.SupplierCode + "</div><div class='supplierName'>" + item.SupplierName + "</div><div class='supplierAddress'>" + item.SupplierAddress + "</div>")
                .appendTo(ul);
            };
    }
    //Hàm thực hiện autocomplete danh sách nhân viên
    ...
    autocompleteEmployee() {
        ...
        $("[fieldname='EmployeeCode']").autocomplete({
            appendTo: '#dialog-add-receipt',
            source: array,
            minLength: 1,
            select: function (event, ui) {
                if ($('#radio1').is(":checked")) {
                    $(".dialog-body-different [fieldname='EmployeeCode']").val(ui.item.EmployeeCode);
                    $(".dialog-body-different [fieldname='EmployeeName']").val(ui.item.EmployeeName);
                    $(".table-detail-body .input-detail-1:first").focus();
                    datatable.employeeIDDifferent = ui.item.EmployeeID;
                }
                if ($('#radio2').is(":checked")) {
                    $(".dialog-body-debt [fieldname='EmployeeCode']").val(ui.item.EmployeeCode);
            ...
            return $("<li class='li-employee'>")
                ...
                .append("<div class='employeeId'>" + item.EmployeeCode + "</div><div class='employeeName'>" + item.EmployeeName + "</div>")
                .appendTo(ul);
        };
    }
    //Hàm autocomplete cho dropdown
    //Người tạo NVMANH 30/7/2019
    autoCompleteItem(input, sources) {
        $(input).autocomplete({
            source: sources,
            minLength: 1,
            select: function (event, ui) {
            ...
            open: function () {
                form.autoCompleteIsOpen= true;
            },
            close: function () {
                form.autoCompleteIsOpen= false;
            }
        });
    }
    //Hàm autocomplete cho loại chưng từ trên bảng chính
    //Người tạo NVMANH 30/7/2019
    autoCompleteTypeVoucher() {
        var source = [
            'Tất cả',
            'Phiếu thu tiền mặt',
            ...
            'Phiếu chi tiền mặt',
            'Phiếu trả nợ - tiền mặt',
            'Phiếu nhập hàng - tiền mặt',
            'Phiếu trả lại hàng mua - tiền mặt'
        ];
        this.autoCompleteItem('.combobox.select-type-voucher-item', source);
    }
    //Hàm autocomplete cho chọn mục thu trong bảng chi tiêt trên dialog
    //Người tạo NVMANH 30/7/2019
    autoCompleteSelectItem() {
        ...
        this.autoCompleteItem('body .input-detail-3', source)
    }
    //Gọi tất cả các hàm autocomplete
    //Người tạo NVMANH 30/7/2019
    autoCompleteAll() {
        this.autoCompleteSelectItem();
        this.autoCompleteTypeVoucher();
    }
    //Hàm bắt sự kiện lọc
    //Người tạo NVMANH 30/7/2019
    evtClickFilter() {
    ...
    getConditionFilter() {
        var condition = [];
        var inputCondition = $('.content-table-header [fieldName]');
        $.each(inputCondition, function (index, item) {
            let itemCondition = {};
            let fieldName = $(this).attr('fieldName');
            if (fieldName === 'FundMoney') {
                itemCondition['Field'] = fieldName;
                var anotation = $(this).parent().prev().find('.btn-header').attr('value');
                switch (anotation) {
                    case '1':
                        itemCondition['Type'] = '=';
                        break;
                ...
                itemCondition['DataType'] = 'decimal';
                itemCondition['Value'] = $(this).val();
            } else if (fieldName === 'FundDate') {
                itemCondition['Field'] = fieldName;
                itemCondition['Type'] = '*';
                itemCondition['DataType'] = 'date';
                itemCondition['Value'] = $(this).val();
            } else {
                itemCondition['Field'] = fieldName;
                itemCondition['Type'] = '*';
                itemCondition['DataType'] = 'text';
                itemCondition['Value'] = $(this).val();
            }
